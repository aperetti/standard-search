<template>
  <div>
    <form class="well form-horizontal" v-bind:action="submitUrl" method="POST" enctype="multipart/form-data" v-on:submit.prevent="onSubmit">
      <div class="page-header"><h2 class="text-left">Edit Standard</h2></div>
      <!-- STANDARD CODE FORM GROUP -->
      <div v-bind:class="['form-group',  $vd.code.$valid ? 'has-success' : 'has-error']">
          ...
          <div class="input-group" >
            <div v-if="addGroup">
              <div class="input-group-btn">
                <input type="text" @keydown.enter="pushGroup(this.newGroup)" v-if="addGroup" id="group-add" class="form-control" placeholder="Group" v-model="newGroup">
        ...
        <div class="col-sm-8 col-xs-10 col-xs-offset-1">
          <div class="input-group-btn">
            <label v-bind:class="['btn', fileUploaded ? 'btn-success' : 'btn-default', this.fileConflict ? 'btn-warning' : 'btn-default', 'btn-file', 'pull-left']">
              <span class="glyphicon glyphicon-cloud-upload"></span> Upload PDF <input id="pdfFile" type="file" name="pdf" style="display: none;">
            </label>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8 col-xs-10 col-xs-offset-1">
          <div class="input-group-btn">
            <template v-for="(group, index) in references">
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit: function (e) {
        ...
        xhr.onload = () => {
          if (xhr.status === 200) {
            this.$routr.push({name: 'standard', params: {standardId: this.$route.params.standardId}})
